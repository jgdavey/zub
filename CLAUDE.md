...
1. **config** — `main.rs` resolves the config path (`-C/--config`, else `$ZUB_CONFIG`, else error) and loads the YAML (`name`, optional `root`/`command_roots`/`version`/`description`). `config::load` returns `Result<Config, LoadError>`, distinguishing a read failure (missing/unreadable file) from a YAML parse error so `main.rs` can report the underlying cause.
