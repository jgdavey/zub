...
1. **config** — `main.rs` resolves the config path (`-C/--config`, else `$ZUB_CONFIG`, else error) and loads the YAML (`name`, optional `root`/`libexec`/`version`/`description`). `config::load` returns `Result<Config, LoadError>`, distinguishing a read failure (missing/unreadable file) from a YAML parse error so `main.rs` can report the underlying cause.
2. **identity** — `resolve(config_path, config)` derives `name` from the config, `root` from the config's `root` field (when set) else the config file's parent dir, `libexec` from the config's `libexec` field (a relative path resolved against `root`, an absolute path used as-is) defaulting to `<root>/libexec`, and detects a per-directory `local_root` (`$PWD/.<name>/libexec`). Produces an `Identity { name, root, libexec, local_root, config_path }`. (The configured `libexec` governs the program's command dir; the per-directory `local_root` override stays the fixed `.<name>/libexec` convention.)
