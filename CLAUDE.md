...
2. **identity** — `resolve(config_path, config)` derives `name` from the config, `root` from the config's `root` field (when set) else the config file's parent dir, and resolves `command_roots` from the config's `command_roots` list (`effective_templates`, defaulting to `["$ZUB_ROOT/libexec", "$ZUB_LOCAL_ROOT/.$ZUB_INSTANCE/libexec"]`). Each entry's `$ZUB_ROOT`/`$ZUB_INSTANCE`/`$PWD`/`$ZUB_LOCAL_ROOT` pseudo-vars are expanded, a still-relative entry is resolved against `root`, and an entry referencing `$PWD` or `$ZUB_LOCAL_ROOT` is flagged `is_local`. `$ZUB_LOCAL_ROOT` is the *project* root: `find_local_root` walks up from `$PWD` (inclusive, to `/`) to the first ancestor containing a `.<name>` **directory** — contents irrelevant, so a nearer empty `.<name>` shadows a fuller one further up. The walk runs only when some template mentions the var, and when it finds nothing those entries are **dropped** (not expanded against an empty path). Produces an `Identity { name, root, command_roots: Vec<CommandRoot { path, is_local }>, local_root: Option<PathBuf>, config_path }`. The list is ordered lowest-precedence first (a later root overrides an earlier one). `Identity::new_command_dir()` (where `new` creates a command) is the first non-local root.
